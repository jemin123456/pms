  ...
  try {
    ...
    if (type === 'user') {
      ...
      if (!tenantName || !tenantSlug) {
        return res.status(400).json({ success: false, message: 'Please provide company name and slug' });
      }

      const slugExists = await Tenant.findOne({ slug: tenantSlug.toLowerCase() });
      if (slugExists) {
        return res.status(400).json({ success: false, message: 'Organization slug/subdomain is already taken' });
      }
...
exports.refresh = async (req, res, next) => {
  try {
    ...
    let decoded;
    try {
      decoded = jwt.verify(refreshToken, process.env.JWT_REFRESH_SECRET || 'supersecretkeyforrefresh_54321');
    } catch (err) {
      return res.status(401).json({ success: false, message: 'Invalid or expired refresh token' });
    }

    const user = await User.findById(decoded.id).populate('memberships.tenantId', 'name slug');
    if (!user) {
      return res.status(401).json({ success: false, message: 'User associated with refresh token not found' });
    }
...
exports.getMe = async (req, res, next) => {
  try {
    const user = await User.findById(req.user.id).populate('memberships.tenantId', 'name slug');
    res.status(200).json({
      success: true,
      data: user,
    });
...
exports.addEmployee = async (req, res, next) => {
  try {
    ...
    if (req.userRole !== 'admin') {
      return res.status(403).json({ success: false, message: 'Access denied: Only administrators can add employees' });
    }

    // Find the user to add
    const employee = await User.findById(userId);
    if (!employee) {
      return res.status(404).json({ success: false, message: 'User not found. Please verify the ID.' });
    }
...
exports.getEmployees = async (req, res, next) => {
  try {
    if (!req.tenantId) {
      return res.status(400).json({ success: false, message: 'No active workspace selected' });
    }
    ...
    const mappedEmployees = employees.map(emp => {
      const memb = emp.memberships.find(m => m.tenantId.toString() === req.tenantId.toString());
      return {
        id: emp._id,
        name: emp.name,
        email: emp.email,
...
exports.createWorkspace = async (req, res, next) => {
  try {
    ...
    if (!name || !slug) {
      return res.status(400).json({ success: false, message: 'Workspace name and slug are required' });
    }

    const slugExists = await Tenant.findOne({ slug: slug.toLowerCase() });
    if (slugExists) {
      return res.status(400).json({ success: false, message: 'Workspace slug is already taken' });
    }
